...
// 左边侧边栏触摸滑动
function leftUlAnimate() {

	function startTransition() {
		mainLeftUl.style.transition = "all .3s linear";
	}
	function closeTransition() {
		mainLeftUl.style.transition = "";
	}
	function setTransform(distance) {
		mainLeftUl.style.transform = "translateY("+distance+"px)";
	}
	// 定义一些必要的值
	var mainLeft = document.querySelector(".container_main_left");
	var mainLeftUl = document.querySelector(".container_main_left ul");
	var mainLeftHeight = mainLeft.offsetHeight;
	var mainLeftUlHeight = mainLeftUl.offsetHeight;
	var maxDistance = 0;
	//因为在css中mainLeft和body一样高 所以会多出一个顶部导航的高度 所以在minDistance中要减去
	var headerHeight = document.querySelector(".container_header").offsetHeight;
	var minDistance = mainLeftHeight - mainLeftUlHeight - headerHeight;
	console.log(minDistance);

	// 定义触摸初始值,移动值,distance作为ul的初始值
	var startY = 0;
	var moveY = 0;
	var distanceY = 0;

	//定义一个用来为吸附效果做准备的delayDistance
	var delayDistance = 100;

	//触摸开始事件
	mainLeftUl.addEventListener("touchstart",function(event) {
		startY = event.touches[0].clientY;
	})

	//触摸移动事件
	mainLeftUl.addEventListener("touchmove",function(event) {
	...
	//触摸结束事件
	mainLeftUl.addEventListener("touchend",function(event) {
		distanceY += moveY;
...
// 	// 获取移动的ul
// 	var moveUl = document.querySelector(".container_main_left ul");

// 	// ul父盒子的高度	
// 	var parentHeight = document.querySelector(".container_main_left").offsetHeight;
